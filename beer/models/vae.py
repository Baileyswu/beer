...
class VAE(Model):
    ...
    def expected_log_likelihood(self, data, nsamples=1, llh_weight=1.,
        ...
        s_samples = self.prior.sufficient_statistics(
            samples.view(-1, samples.shape[-1]))
        s_samples = s_samples.reshape(
            len(samples), -1, s_samples.shape[-1]).mean(dim=1)
        ...
        r_data = data[:, None, :].repeat(
            1, nsamples, 1).view(-1, data.shape[-1])
...
class MvVAE(Model):
    def __init__(self, prior, enc1, dec1, enc2, dec2):
        ...
        self.enc_mean_layer = torch.nn.Linear(
            enc1.dim_out + enc2.dim_out, dec1.dim_in)
        self.enc_var_layer = torch.nn.Linear(
            enc1.dim_out + enc2.dim_out, dec1.dim_in)
    ...
    def llh_in_view(self, view_n, samples, data, n_samples):
        ...
        r_data = data[:, None, :].repeat(
            1, n_samples, 1).view(-1, data.shape[-1])
        llh = pdfs(pdfs.sufficient_statistics(r_data), pdfwise=True)
    ...
    def expected_log_likelihood(self, s_stats, n_samples=1, llh_weight=1., kl_weight=1., **kwargs):
        ...
        # Local KL-divergence.
        ent = -posts(posts.sufficient_statistics(
                samples).mean(dim=1), pdfwise=True)
        s_samples = self.prior.sufficient_statistics(
            samples.view(-1, samples.shape[-1]))
        s_samples = s_samples.reshape(
            len(samples), -1, s_samples.shape[-1]).mean(dim=1)
